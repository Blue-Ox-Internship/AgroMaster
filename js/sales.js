...
async function initSales() {
  ...
  document.getElementById('export-csv-btn').addEventListener('click', () => {
    const headers = ['Medicine Name', 'Category', 'Qty Sold', 'Unit Price', 'Total Amount', 'Sale Date'];
    const rows = filteredSales.map(s => {
      ...
      return [
        ...
        s.selling_price,
        s.total_amount,
        s.sale_date ? App.formatDate(s.sale_date) : ''
      ];
    });
...
async function loadSalesStats() {
  let salesList = [];
  try {
    salesList = await API.getSales();
  } catch (err) {
    salesList = DB.getSales();
  }
  const today = App.today();
  const todayS = salesList.filter(s => s.sale_date === today);
  const todayTotal = todayS.reduce((sum, s) => sum + (s.total_amount || 0), 0);
  const now = new Date();
  const monthS = salesList.filter(s => { const d = new Date(s.sale_date); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); });
  ...
  document.getElementById('sales-stats').innerHTML = `
    ...
    <div class="stat-card blue"><div class="stat-icon blue"><i class="fas fa-receipt"></i></div><div class="stat-info"><h3>${salesList.length}</h3><p>Total Sales Records</p></div></div>`;
}
...
function renderTable() {
  ...
  App.renderPagination('pagination', currentPage, filteredSales.length, PAGE_SIZE, goPage);
}
...
function openSaleModal() {
  ...
  const meds = allMeds.filter(m => m.quantity > 0 && !App.isExpired(m.expiry_date));
...
async function saveSale() {
  ...
  await API.addSale({ medicine_id: medId, quantity: qty, selling_price: price, sale_date: date });
  Toast.show('success', 'Sale Recorded!', `${qty} unit(s) of ${med.medicine_name} sold for ${App.formatCurrency(qty * price)}.`);
  Modal.close('sale-modal');
  await loadSalesStats();
  await loadSales();
}

function viewSale(id) {
  const s = allSales.find(x => x.sale_id === id);
  if (!s) return;
  const med = allMeds.find(m => m.medicine_id === s.medicine_id);
