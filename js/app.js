...
const App = {
    ...
    populateSidebar() {
        ...
        const nameEl = document.querySelector('.sidebar-user .user-info h4');
        ...
        // Topbar user
        const topAvatar = document.querySelector('.topbar-user .avatar');
        const topName = document.querySelector('.topbar-user .user-info h4');
        const topRole = document.querySelector('.topbar-user .user-info span');

        if (topAvatar) topAvatar.textContent = user.full_name.charAt(0).toUpperCase();
        if (topName) topName.textContent = user.full_name;
        if (topRole) topRole.textContent = user.role;

        // Hide admin-only nav items if not admin
